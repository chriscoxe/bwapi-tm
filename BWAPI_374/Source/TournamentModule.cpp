...
TMState::TMState() :
    stateFileVer("3.0.0"),
    exceededTimeoutsLimit(0),
    frameCountTimeoutsExceeded(0),
    frameCount(0),
    exceededFrameCountLimit(0),
    frameCountFrameCountLimitExceeded(0),
    selfKillScore(0),
    selfBuildingScore(0),
    selfRazingScore(0),
    selfUnitScore(0),
    selfCustomScore(0),
    selfGatheredMinerals(0),
    selfSpentMinerals(0),
    selfRepairedMinerals(0),
    selfRefundedMinerals(0),
    selfGatheredGas(0),
    selfSpentGas(0),
    selfRepairedGas(0),
    selfRefundedGas(0),
    ...
    enemyScore(0),
    gameElapsedTime(0),
    exceededElapsedTimeLimit(0),
    frameCountElapsedTimeExceeded(0),
    defeated(0),
    victorious(0),
    gameOver(0),
    isWinnerInOnEnd(0),
...
TMAI::TMAI() :
    settingsFileName("bwapi-data\\tm_settings.ini"),
    stateFileName("gameState.txt"),
    gameFrameLimit(85714),
    gameElapsedTimeLimit(0),
    frameThreshTimed(11),
    stateFileUpdateTimePeriod(5000),
    stateFileUpdateFramePeriod(0),
    totFrameTime(0),
    avgFrameTime(0),
    maxFrameTime(0),
    minFrameTime(999999),
    ...
    disableUserInputLocalSpeed(true),
    drawUnitInfo(true),
    drawTournamentInfo(true),
    drawBotNames(true),
    disableDrawGameTimer(true),
    drawGUI(true),
    autoObs(true),
    autoResumeGame(true)
{
...
void TMAI::onStart()
{
    ...
    if (drawTournamentInfo)
    {
        ...
        for (std::vector<std::string>::iterator iter = diagnostics.begin(); iter != diagnostics.end(); ++iter)
        {
            BWAPI::Broodwar->printf("%s", iter->c_str());
        }
    }
...
void TMAI::onPlayerDropped(BWAPI::Player* player)
{
    frameTimes[BWAPI::Broodwar->getFrameCount()] += BWAPI::Broodwar->getLastEventTime();
    if (autoObs)
    {
        autoObserver.onPlayerDropped(player);
    }
}

bool TM::onAction(int actionType, void *parameter)
{
    switch ( actionType )
    {
